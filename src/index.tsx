...
import { LoadingOverLayProps, LoadingOverlayState, OverflowCSS } from './LoadingOverlayTypes'

export { LoadingOverLayProps } from './LoadingOverlayTypes'

class LoadingOverlayWrapperBase extends PureComponent<LoadingOverLayProps, LoadingOverlayState> {
  ...
  componentDidMount() {
    ...
    if (this.wrapperEl !== null && this.wrapperEl !== undefined) {
      ...
      const overflowCSS = (['overflow', 'overflowX', 'overflowY'] as Array<keyof OverflowCSS>).reduce<OverflowCSS>((m, i) => {
   ...
   * the base css obj.
   */
  getStyles = (key: keyof Styles, providedState?: OverflowCSS | TransitionStatus) => {
      ...
      <div
        ...
        className={
          this.cx(
            ...
            css(this.getStyles('wrapper', active ? overflowCSS : undefined) as CSSInterpolation),
            className
          )
        }
      >
        ...
        >
          {state => (
            <div
              ref={this.overlayRef}
              data-testid='overlay'
              className={this.cx('overlay', css(this.getStyles('overlay', state) as CSSInterpolation))}
              onClick={onClick}
            >
              <div className={this.cx('content', css(this.getStyles('content') as CSSInterpolation))}>
                {spinner && (
                  typeof spinner === 'boolean'
                    ? <Spinner cx={this.cx} getStyles={this.getStyles as SpinnerProps['getStyles']} />
                    : spinner
                )}
                {text}
              </div>
...
const LoadingOverlayWrapper = forwardRef<HTMLDivElement, LoadingOverLayProps>((props, ref) => 
  <LoadingOverlayWrapperBase innerRef={ref} {...props} />
)
